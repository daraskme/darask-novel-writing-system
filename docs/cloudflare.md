...
同じCloudflareの校閲URLで、索引の「校閲する版」からPRを選びます。PRでは追加・変更された話だけを表示します。本文の青色・下線が追加・書き換え部分で、「次の変更へ」で順に移動できます。「変更前・削除した文章を見る」を開くと、以前の文章と削除部分を赤色・取り消し線で確認できます。比較元はPRの分岐元です。大きな変更は段落単位で強調します。指示の引用は現在のPR本文だけを記録します。`/reader/?pr=7#1` のように直接開けますが、その話が変更されていない場合は最初の変更話を開きます。通常版では全話を表示します。
