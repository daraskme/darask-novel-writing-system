...
const review = { number: '', pull: null, serial: 0, listSerial: 0 };
...
async function loadReviewIndex(loadStatic) {
  ...
  try {
    ...
    const first = requested || (review.pull && episodes.find(e => e.changed));
    ...
    else status(review.pull ? 'PRで変更された本文はありません。索引から前後の話も読めます。' : '');
  } catch (error) {
