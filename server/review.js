  ...
  return async ({ request, env }) => {
    ...
    try {
      ...
      if (path) {
        ...
        return json({ repo, text: file.text, sha: file.sha, hash, commit: pr.head.sha });
      }
      ...
      return json({ repo, pull: summarize(pr), episodes: episodes.map(e => ({ ...e, changed: changed.has(e.manuscript) })) });
    } catch (error) {
